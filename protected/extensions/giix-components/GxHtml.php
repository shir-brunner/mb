...
class GxHtml extends CHtml {
	...
	//TODO: what makes it a date field?
	public static function activeDateField($model, $attribute, $htmlOptions = array())
	{
		$value = null;
		$dateFormat = Yii::app()->user->getState('user')->dateFormat;
		$dateTime = DateTime::createFromFormat('Y-m-d', $model->$attribute);

		if($model->$attribute && $dateTime)
		{
			if($dateFormat->getPrimaryKey() == DateFormat::DMY)
			{
				$value = $dateTime->format('d/m/Y');
			}
			else if($dateFormat->getPrimaryKey() == DateFormat::MDY)
			{
				$value = $dateTime->format('m/d/Y');
			}
		}

		$htmlOptions['data-mask'] = '99/99/9999';
		$htmlOptions['placeholder'] = $dateFormat->date_format_name;
		$htmlOptions['class'] = isset($htmlOptions['class']) ? $htmlOptions['class'] . ' date-field' : 'date-field';

		$name = get_class($model) . '[' . $attribute . ']';

		return GxHtml::textField($name, $value, $htmlOptions);
	}

	//TODO: what makes it a time field?
	public static function activeTimeField($model, $attribute, $htmlOptions = array())
	{
		$htmlOptions['readonly'] = 'readonly';
		return GxHtml::textField($model, $model->$attribute, $htmlOptions);
	}

	public static function activeDropDownListAjax($model, $attribute, $data, $htmlOptions = array())
	{
		if($model->isNewRecord)
		{
			throw new Exception('GxHtml::activeDropDownListAjax does not support new records.');
		}

		$htmlOptions['class'] = isset($htmlOptions['class']) ? ($htmlOptions['class'] . ' ajax-field') : 'ajax-field';

		$urlController = lcfirst(get_class($model));
		$htmlOptions['ajax-url'] = Yii::app()->createUrl($urlController . '/update', array('id' => $model->getPrimaryKey()));

		unset($htmlOptions['name']); //prevent anyone from changing the name because it is used by the ajax action

		return parent::activeDropDownList($model, $attribute, $data, $htmlOptions);
	}

	public static function activeDateFieldAjax($model, $attribute, $htmlOptions = array())
	{
		$value = null;
		$dateFormat = Yii::app()->user->getState('user')->dateFormat;
		$dateTime = DateTime::createFromFormat('Y-m-d', $model->$attribute);
		...
		$htmlOptions['class'] = isset($htmlOptions['class']) ? $htmlOptions['class'] . ' ajax-date' : 'ajax-date';

		return self::activeTextFieldAjax($model, $attribute, $htmlOptions);
	}

	public static function activeWeightFieldAjax($model, $attribute, $htmlOptions = array())
	{
		$htmlOptions['placeholder'] = isset($htmlOptions['placeholder']) ? $htmlOptions['placeholder'] : UnitSystem::weightLabel();
		if($model->$attribute && Yii::app()->user->getState('user')->unit_system_id == UnitSystem::METRIC)
		{
			$model->$attribute = UnitSystem::convertWeightMetric($model->$attribute);
		}

		$model->$attribute = round($model->$attribute, 1);
		return self::activeTextFieldAjax($model, $attribute, $htmlOptions);
	}

	public static function activeLengthFieldAjax($model, $attribute, $htmlOptions = array())
	{
		$htmlOptions['placeholder'] = isset($htmlOptions['placeholder']) ? $htmlOptions['placeholder'] : UnitSystem::lengthSmallLabel();
		if($model->$attribute && Yii::app()->user->getState('user')->unit_system_id == UnitSystem::METRIC)
		{
			$model->$attribute = UnitSystem::convertLengthMetric($model->$attribute);
		}
	...
	public static function activeFluidFieldAjax($model, $attribute, $htmlOptions = array())
	{
		$htmlOptions['placeholder'] = isset($htmlOptions['placeholder']) ? $htmlOptions['placeholder'] : UnitSystem::fluidLabel();
		if($model->$attribute &&  Yii::app()->user->getState('user')->unit_system_id == UnitSystem::METRIC)
		{
			$model->$attribute = UnitSystem::convertFluidMetric($model->$attribute);
		}
	...
	public static function activeWeightField($model, $attribute, $htmlOptions = array())
	{
		$htmlOptions['placeholder'] = isset($htmlOptions['placeholder']) ? $htmlOptions['placeholder'] : UnitSystem::weightLabel();
		if($model->$attribute &&  Yii::app()->user->getState('user')->unit_system_id == UnitSystem::METRIC)
		{
	...
	public static function activeLengthField($model, $attribute, $htmlOptions = array())
	{
		$htmlOptions['placeholder'] = isset($htmlOptions['placeholder']) ? $htmlOptions['placeholder'] : UnitSystem::lengthSmallLabel();
		if($model->$attribute &&  Yii::app()->user->getState('user')->unit_system_id == UnitSystem::METRIC)
		{
	...
	public static function activeFluidField($model, $attribute, $htmlOptions = array())
	{
		$htmlOptions['placeholder'] = isset($htmlOptions['placeholder']) ? $htmlOptions['placeholder'] : UnitSystem::fluidLabel();
		if($model->$attribute &&  Yii::app()->user->getState('user')->unit_system_id == UnitSystem::METRIC)
		{
	...
	public static function activeTextFieldAjax($model, $attribute, $htmlOptions = array())
	{
		if($model->isNewRecord)
		{
			throw new Exception('GxHtml::activeTextFieldAjax does not support new records.');
		}
	...
	public static function activeTextAreaAjax($model, $attribute, $htmlOptions = array())
	{
		if($model->isNewRecord)
		{
			throw new Exception('GxHtml::activeTextAreaAjax does not support new records.');
		}
	...
	//TODO: rename to activeCheckboxAjax (small 'b')
	public static function activeCheckBoxAjax($model,$attribute,$htmlOptions=array())
	{
		if($model->isNewRecord)
		{
			throw new Exception('GxHtml::activeCheckBoxAjax does not support new records.');
		}
