...
$deletedColumns = array_filter($columns, function($column) {
	return StringHelper::endsWith($column->name, 'deleted');
});

$deletedColumn = reset($deletedColumns);

$createTimeColumns = array_filter($columns, function($column) {
	return StringHelper::endsWith($column->name, 'create_time');
});

$createTimeColumn = reset($createTimeColumns);
...
abstract class <?php echo $this->baseModelClass; ?> extends <?php echo $this->baseClass; ?> {
	...
	{
		return '<?php echo strtolower($tableName); ?>';
	}
...
<?php $partial = ($column->type === 'string' and !$column->isForeignKey); ?>
		$criteria->compare('<?php echo ($column->isForeignKey ? 't.' : '') . $name; ?>', $this-><?php echo $name; ?><?php echo $partial ? ', true' : ''; ?>);
<?php endforeach; ?>
...
<?php if($deletedColumn) { ?>
	...
	{
		return $this->saveAttributes(array('<?php echo $deletedColumn->name; ?>' => 1));
	}

	public function undoDelete()
	{
		return $this->saveAttributes(array('<?php echo $deletedColumn->name; ?>' => 0));
	}
	...
	public function defaultScope()
	{
		...
		return array('condition' => "$alias.<?php echo $deletedColumn->name; ?> = 0");
	}
<?php } ?>
<?php if($createTimeColumn) { ?>
	protected function beforeValidate()
	{
		if($this->isNewRecord && empty($this-><?php echo $createTimeColumn->name; ?>))
		{
			$this->setAttribute('<?php echo $createTimeColumn->name; ?>', date('Y-m-d H:i:s'));
		}
